		...
		<div class="row" id="printable">
            ...
            <div class="row">
                <div class="col-md-4">
                    ...
                    <p style="margin-left: 10px;"><?= slashedDate($overtime->created_at) ?></p>
                </div>
			...
			if((Auth::user()->isAdmin() == 1 || $overtime->supervisor_id == Auth::user()->id) && empty($overtime->recommend_date) && $overtime->status == 'PENDING') {
			?>
			...
			<?php
				...
				if((Auth::user()->usertype == 3 && $overtime->status != 'PENDING') || Auth::user()->isAdmin()) {
			?>
					<a href="<?= url("overtime/{$overtime->id}/edit") ?>" class="btn btn-info">Update</a>
			<?php
				}

				if($overtime->status == 'PENDING' || $overtime->status == 'DECLINED') {
			?>
